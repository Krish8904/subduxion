...
export default function ExpenseCurrency() {
  ...
  useEffect(() => {
    ...
    setFiltered(currencies.filter(c => 
  ...
  // Modal handlers
  const openCreate = () => {
    setEditingItem(null); setLabel(""); setValue(""); setCode(""); setError(""); setShowModal(true);
  };
  const openEdit = (item) => {
    setEditingItem(item); setLabel(item.label); setValue(item.value); setCode(item.code || ""); setError(""); setShowModal(true);
  };
  const closeModal = () => {
    setShowModal(false); setEditingItem(null); setLabel(""); setValue(""); setCode(""); setError("");
  };
  const openDelete = (item) => { setDeletingItem(item); setShowDeleteModal(true); };

  // Save currency
  const handleSave = async () => {
    if (!label.trim() || !value.trim()) { setError("Label and Value cannot be empty"); return; }
    setSaving(true); setError("");
    try {
      const payload = { label: label.trim(), value: value.trim(), code: code.trim() };
      if (editingItem) {
        await axios.put(`${API}/currency/${editingItem._id}`, payload);
        showToast("Currency updated successfully");
      } else {
        await axios.post(`${API}/currency`, payload);
      ...
      if (msg.includes("duplicate") || msg.includes("E11000")) setError("This currency already exists");
      else setError(msg);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`${API}/currency/${deletingItem._id}`);
      showToast("Currency deleted successfully");
      setShowDeleteModal(false); setDeletingItem(null);
      fetchCurrencies();
    } catch {
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-4xl font-bold text-slate-800">Manage Currencies</h1>
          <p className="text-slate-500 text-sm mt-1">
            {currencies.length} {currencies.length === 1 ? "currency" : "currencies"} total
          </p>
        </div>
        <button onClick={openCreate} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow font-medium transition flex items-center gap-2">
        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            <div key={item._id} className="bg-white rounded-2xl shadow-sm border p-6 hover:shadow-md transition group">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Value: {item.value}</p>
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-lg font-medium">{search ? "No results found" : "No currencies added yet"}</p>
              <p className="text-sm mt-1">{search ? "Try a different search term" : 'Click "+ Add Currency" to get started'}</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-110 rounded-2xl p-6 shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-semibold text-slate-800">{editingItem ? "Edit Currency" : "Add Currency"}</h2>
            ...
            <input autoFocus value={label} onChange={(e) => { setLabel(e.target.value); setError(""); }} onKeyDown={handleKeyDown} placeholder="e.g. US Dollar" className={`w-full border rounded-xl p-3 focus:ring-2 outline-none transition ${error ? "border-red-400 focus:ring-red-300" : "border-slate-200 focus:ring-blue-500"}`} />

            <label className="block text-sm font-medium text-slate-600 mb-1.5 mt-4">Value <span className="text-red-500">*</span></label>
            <input value={value} onChange={(e) => { setValue(e.target.value); setError(""); }} onKeyDown={handleKeyDown} placeholder="USD" className={`w-full border rounded-xl p-3 focus:ring-2 outline-none transition ${error ? "border-red-400 focus:ring-red-300" : "border-slate-200 focus:ring-blue-500"}`} />

            <label className="block text-sm font-medium text-slate-600 mb-1.5 mt-4">Code</label>
            <input value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={handleKeyDown} placeholder="Optional" className="w-full border rounded-xl p-3 focus:ring-2 outline-none transition border-slate-200 focus:ring-blue-500" />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-100 rounded-2xl p-6 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-slate-800">Delete Currency</h2>
            </div>
