...
export default function ChannelMasterPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await axios.put(`${API}/channel/${editingItem._id}`, { name: name.trim() });
        showToast("Channel updated successfully");
      } else {
        await axios.post(`${API}/channel`, { name: name.trim() });
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`${API}/channel/${deletingItem._id}`);
    ...
    <div className="font-['Poppins'] min-h-screen bg-slate-50/80 px-9 py-11">
      ...
      <div className="animate-fade-up mb-9">
        <div className="flex items-baseline gap-2.5 mb-1">
          <h1 className="text-4xl font-black text-slate-900 -tracking-1 leading-none">Channels</h1>
      ...
      {loading ? (
        ...
        <div className="animate-fade-up flex flex-col items-center justify-center py-20 text-slate-400">
          ...
          <p className="text-lg font-semibold text-slate-500 mb-0">
            {search ? "No results found" : "No channels added yet"}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
          {filtered.map((item) => (
            <div key={item._id} className="channel-card bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
        ...
        <div className="fixed inset-0 bg-slate-900/45 backdrop-blur-sm flex items-center justify-center z-50 p-5">
          <div className="bg-white w-full max-w-md rounded-3xl p-7 pb-6 shadow-2xl animate-modal-in">
            <div className="flex items-center justify-between mb-5.5">
              <div>
                ...
                <p className="text-xs text-slate-400 mb-0">
                  {editingItem ? `Editing: ${editingItem.name}` : "Create a new channel entry"}
                </p>
              </div>
              <button 
                onClick={closeModal} 
            ...
            <input 
              ...
              placeholder="e.g. Direct Sales, Online..."
            />
