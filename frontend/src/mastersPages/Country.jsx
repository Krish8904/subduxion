...
export default function AdminCountriesPage() {
  ...
  const openCreate = () => {
    setEditingItem(null);
    setLabel(""); setValue(""); setCode(""); setError("");
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await axios.put(`${API}/country/${editingItem._id}`, payload);
        showToast("Country updated successfully");
      } else {
        await axios.post(`${API}/country`, payload);
      ...
      if (msg.includes("duplicate") || msg.includes("E11000")) setError("This country already exists");
      else setError(msg);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`${API}/country/${deletingItem._id}`);
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-4xl font-bold text-slate-800">Manage Countries</h1>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-lg font-medium">{search ? "No results found" : "No countries added yet"}</p>
              <p className="text-sm mt-1">{search ? "Try a different search term" : 'Click "+ Add Country" to get started'}</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-110 rounded-2xl p-6 shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-semibold text-slate-800">{editingItem ? "Edit Country" : "Add Country"}</h2>
            ...
            <input autoFocus value={label} onChange={(e) => { setLabel(e.target.value); setError(""); }} onKeyDown={handleKeyDown} placeholder="e.g. India" className={`w-full border rounded-xl p-3 focus:ring-2 outline-none transition ${error ? "border-red-400 focus:ring-red-300" : "border-slate-200 focus:ring-blue-500"}`} />

            <label className="block text-sm font-medium text-slate-600 mb-1.5 mt-4">Value <span className="text-red-500">*</span></label>
            <input value={value} onChange={(e) => { setValue(e.target.value); setError(""); }} onKeyDown={handleKeyDown} placeholder="IN" className={`w-full border rounded-xl p-3 focus:ring-2 outline-none transition ${error ? "border-red-400 focus:ring-red-300" : "border-slate-200 focus:ring-blue-500"}`} />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-100 rounded-2xl p-6 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-slate-800">Delete Country</h2>
            </div>
